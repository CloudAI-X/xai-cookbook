...
def get_test_command() -> list[str] | None:
    ...
    # Python projects - use uv if available, otherwise pytest directly
    if (cwd / "pytest.ini").exists() or (cwd / "pyproject.toml").exists():
        # Check if uv is available and project uses it
        if (cwd / "uv.lock").exists():
            return ["uv", "run", "python", "-m", "pytest", "--tb=no", "-q"]
...
def send_notification(title: str, message: str, is_error: bool = False):
    ...
    # Linux
        ...
        subprocess.run(["notify-send", title, message, f"--urgency={urgency}"], capture_output=True)
...
def main():
    ...
    if test_cmd:
        ...
        elif "no tests ran" in test_output.lower() or "no tests collected" in test_output.lower():
            results.append("✓ Tests: none configured")
        else:
